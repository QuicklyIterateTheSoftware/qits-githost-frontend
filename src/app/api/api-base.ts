 ...
 *
 * The SPA is served at `/` on `githost.<env>.<domain>` by qits-githost itself, which also answers
 * `/githost/api/…` on that same host — so a same-origin absolute path is not a shortcut, it is what
 * keeps these reads free of CORS and of any credential. A configured base URL would move them cross-origin and buy nothing.
 *
