 ...
 *
 * It renders *nothing* when the state is ready, and the caller renders the content itself behind
 * its own `@if`. That split is what keeps a failed panel from erasing the page around it — the
 * error is drawn where the table would be, and the heading stays where it is.
 *
...
export class Async {
  /** The node's own state. Ready draws nothing here. */
  ...
  /** What failed, in the caller's words: “Could not load the repositories”. */
